...
const schemaPath = path.join(repoRoot, "contract/projection/projection-thread-card-stream.schema.json");
...
if (schema?.properties?.schemaVersion?.const !== 1) {
  throw new Error("projection thread card schemaVersion must be 1");
}

const swift = `// Generated by scripts/generate-dock-thread-card-contract.mjs.
// Source contract: contract/projection/projection-thread-card-stream.schema.json
