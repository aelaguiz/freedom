...
const outputPath = path.join(repoRoot, "CodexDock/AppServer/DockThreadCardDTO.swift");

const schema = JSON.parse(fs.readFileSync(schemaPath, "utf8"));
if (schema?.properties?.schemaVersion?.const !== 2) {
  throw new Error("dock thread card schemaVersion must be 2");
}

const swift = `// Generated by scripts/generate-dock-thread-card-contract.mjs.
...
if (process.argv.includes("--check")) {
  ...
  if (current !== swift) {
    console.error(`${path.relative(repoRoot, outputPath)} is stale. Run: rtk npm run contract:generate`);
    process.exit(1);
  }
  console.log("generated DockThreadCardDTO.swift is current");
} else {
