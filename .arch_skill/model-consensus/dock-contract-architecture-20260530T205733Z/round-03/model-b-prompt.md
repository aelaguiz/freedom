You are Model B, GPT 5.5 xhigh, in the final signoff round of the Codex Dock contract-architecture consensus.

Do not edit files, run tools, or re-open the repo. Use the prior two rounds and the candidate below only. This round is for final alignment, not fresh discovery.

The user requires the final doc to include an exhaustive affected-location checklist. Opus completed a hardened checklist in Round 2 and found important additions: hidden DEBUG fixtures/previews, separate schema-version categories, Archive still on the legacy direct path, and the LLM-labels collision. Your Round 2 already agreed with the core architecture but preferred the canonical name `DockRow`.
...
6. Scope refinements from Opus Round 2:
...
7. Final doc will use the exhaustive five-bucket checklist:
