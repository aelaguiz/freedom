...
SIM_UI_CONTROLLED_MATRIX_SCENARIOS ?= archive-toggle detail-reconnect detail-history-request large-list-checkpoint thread-activity server-rename-notification server-request source-refresh live-lease-expiry multi-host-isolation spawn-edge resync-gap rapid-mutations detail-replay-pressure current-work-visible root-catchup-window-contract mutation-ack-projection-refresh-failure foreground-resume-all-surfaces
