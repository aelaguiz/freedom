...
SIM_UI_SYNC_DURATION_MS ?= 65000
SIM_UI_SYNC_SAMPLE_MS ?= 500
SIM_UI_SYNC_RELAY_DURATION_MS ?= 180000
...
SIM_UI_CONTROLLED_MATRIX_SCENARIOS ?= detail-history-request large-list-checkpoint thread-activity server-request source-refresh live-lease-expiry multi-host-isolation spawn-edge resync-gap rapid-mutations
SIM_UI_CONTROLLED_MATRIX_PASSES ?= 1
...
SIM_UI_ISOLATED_APP_SERVER_PORT ?= 4520
SIM_UI_ISOLATED_RELAY_PORT ?= 4521
SIM_UI_ISOLATED_APP_SERVER_LABEL ?= com.aelaguiz.codex-dock.app-server.isolated.$(SIM_UI_ISOLATED_RUN_ID)
...
app-test: services
	...
	@rtk sh -c 'set -eu; udid="$$(python3 scripts/sim.py resolve "$(SIM)")"; log_dir="$(APP_BUILD_LOG_DIR)"; test_log="$$log_dir/app-test-$(APP_BUILD_NUMBER).log"; test_only_args=""; if [ -n "$(APP_TEST_ONLY)" ]; then test_only_args="-only-testing:$(APP_TEST_ONLY)"; fi; mkdir -p "$$log_dir"; echo "testing $(APP_SCHEME) on simulator $$udid build $(APP_BUILD_NUMBER)"; if ! CODEX_DOCK_UI_TEST_HOSTS="$(CODEX_DOCK_UI_TEST_HOSTS)" CODEX_DOCK_SIM_UI_SYNC_OUT="$(CODEX_DOCK_SIM_UI_SYNC_OUT)" CODEX_DOCK_SIM_UI_SYNC_DURATION_MS="$(CODEX_DOCK_SIM_UI_SYNC_DURATION_MS)" CODEX_DOCK_SIM_UI_SYNC_SAMPLE_MS="$(CODEX_DOCK_SIM_UI_SYNC_SAMPLE_MS)" xcodebuild -quiet test -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" CURRENT_PROJECT_VERSION="$(APP_BUILD_NUMBER)" $$test_only_args > "$$test_log" 2>&1; then echo "app test failed; see $$test_log" >&2; tail -n 80 "$$test_log" >&2; exit 1; fi; echo "tested $(APP_SCHEME) on simulator $$udid build $(APP_BUILD_NUMBER)"'

sim-ui-sync-proof: services
	@rtk python3 scripts/sim.py boot "$(SIM)"
	@rtk xcodegen generate --spec project.yml
	@rtk sh -c 'set -eu; udid="$$(python3 scripts/sim.py resolve "$(SIM)")"; output_dir="$(SIM_UI_SYNC_DIR)"; mkdir -p "$$output_dir" "$(APP_BUILD_LOG_DIR)"; relay_json="$$output_dir/relay-client-path.json"; relay_md="$$output_dir/relay-client-path.md"; ui_samples="$$output_dir/ui-samples.jsonl"; ready_file="$$output_dir/ui-ready.json"; report_json="$$output_dir/simulator-ui-sync.json"; report_md="$$output_dir/simulator-ui-sync.md"; build_log="$(APP_BUILD_LOG_DIR)/sim-ui-sync-build-$(APP_BUILD_NUMBER).log"; test_log="$(APP_BUILD_LOG_DIR)/sim-ui-sync-test-$(APP_BUILD_NUMBER).log"; relay_log="$$output_dir/relay-client-path.out.log"; config="/tmp/codex-client/codex-dock-sim-ui-sync-config.json"; rtk python3 -c "import datetime,json,sys; expires=(datetime.datetime.now(datetime.timezone.utc)+datetime.timedelta(minutes=10)).replace(microsecond=0).isoformat().replace(\"+00:00\",\"Z\"); open(sys.argv[1],\"w\").write(json.dumps({\"outputPath\":sys.argv[2],\"hosts\":sys.argv[3],\"durationMS\":int(sys.argv[4]),\"sampleMS\":int(sys.argv[5]),\"readyPath\":sys.argv[6],\"checkpointSweep\":sys.argv[7]==\"1\",\"expiresAt\":expires})+\"\\n\")" "$$config" "$$ui_samples" "$(SIM_UI_SYNC_HOSTS)" "$(SIM_UI_SYNC_DURATION_MS)" "$(SIM_UI_SYNC_SAMPLE_MS)" "$$ready_file" "$(SIM_UI_SYNC_CHECKPOINT_SWEEP)"; echo "building displayed UI sync proof test for $(APP_SCHEME) on simulator $$udid build $(APP_BUILD_NUMBER)"; if ! xcodebuild -quiet build-for-testing -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" CURRENT_PROJECT_VERSION="$(APP_BUILD_NUMBER)" > "$$build_log" 2>&1; then echo "sim UI sync build failed; see $$build_log" >&2; tail -n 80 "$$build_log" >&2; rm -f "$$config"; exit 1; fi; echo "writing simulator UI sync proof to $$output_dir"; rtk node scripts/dock-relay-sync-audit.mjs --relay-url "$(SIM_UI_SYNC_RELAY_WS)" $(SIM_UI_SYNC_CODEX_HOME_ARG) --mode soak --client-path-only --force-dock-resync --duration-ms "$(SIM_UI_SYNC_RELAY_DURATION_MS)" --sample-interval-ms "$(SIM_UI_SYNC_RELAY_SAMPLE_MS)" --settle-ms 1000 --dock-collection-timeout-ms 120000 --stream-compare-attempts 5 --stream-compare-delay-ms 1000 --max-stream-lag-ms "$(MAX_UI_LAG_MS)" --detail sampled --detail-limit 5 --detail-observe-ms 100 --json-out "$$relay_json" --summary-out "$$relay_md" --summary-only > "$$relay_log" 2>&1 & relay_pid="$$!"; cleanup() { status="$${1:-$$?}"; rm -f "$$config"; if kill -0 "$$relay_pid" 2>/dev/null; then kill "$$relay_pid" 2>/dev/null || true; wait "$$relay_pid" 2>/dev/null || true; fi; exit "$$status"; }; trap cleanup INT TERM; echo "running displayed UI sampler on $(SIM) with hosts=$(SIM_UI_SYNC_HOSTS)"; if ! xcodebuild -quiet test-without-building -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" -only-testing:CodexDockUITests/CodexDockDisplayedSyncProofTests/testSamplesRelayBackedDockDisplayOverTime > "$$test_log" 2>&1; then echo "sim UI sync test failed; see $$test_log" >&2; tail -n 120 "$$test_log" >&2; cleanup 1; fi; echo "waiting for relay sync audit recorder"; wait "$$relay_pid" || { echo "relay sync audit recorder failed; see $$relay_log" >&2; tail -n 120 "$$relay_log" >&2; rm -f "$$config"; exit 1; }; rm -f "$$config"; trap - INT TERM; rtk node scripts/dock-relay-simulator-ui-sync-proof.mjs --relay-report "$$relay_json" --ui-samples "$$ui_samples" --max-ui-lag-ms "$(MAX_UI_LAG_MS)" --json-out "$$report_json" --summary-out "$$report_md" --fail-on-diff; echo "wrote $$report_json"; echo "wrote $$report_md"'

sim-ui-scenario-sync-proof: services
	@rtk python3 scripts/sim.py boot "$(SIM)"
	@rtk xcodegen generate --spec project.yml
	@rtk sh -c 'set -eu; udid="$$(python3 scripts/sim.py resolve "$(SIM)")"; output_dir="$(SIM_UI_SYNC_DIR)"; mkdir -p "$$output_dir" "$(APP_BUILD_LOG_DIR)"; relay_json="$$output_dir/relay-client-path.json"; relay_md="$$output_dir/relay-client-path.md"; ui_samples="$$output_dir/ui-samples.jsonl"; ready_file="$$output_dir/ui-ready.json"; report_json="$$output_dir/simulator-ui-sync.json"; report_md="$$output_dir/simulator-ui-sync.md"; build_log="$(APP_BUILD_LOG_DIR)/sim-ui-scenario-sync-build-$(APP_BUILD_NUMBER).log"; test_log="$(APP_BUILD_LOG_DIR)/sim-ui-scenario-sync-test-$(APP_BUILD_NUMBER).log"; relay_log="$$output_dir/relay-client-path.out.log"; config="/tmp/codex-client/codex-dock-sim-ui-sync-config.json"; rm -f "$$ready_file"; rtk python3 -c "import datetime,json,sys; expires=(datetime.datetime.now(datetime.timezone.utc)+datetime.timedelta(minutes=10)).replace(microsecond=0).isoformat().replace(\"+00:00\",\"Z\"); open(sys.argv[1],\"w\").write(json.dumps({\"outputPath\":sys.argv[2],\"hosts\":sys.argv[3],\"durationMS\":int(sys.argv[4]),\"sampleMS\":int(sys.argv[5]),\"readyPath\":sys.argv[6],\"checkpointSweep\":sys.argv[7]==\"1\",\"expiresAt\":expires})+\"\\n\")" "$$config" "$$ui_samples" "$(SIM_UI_SYNC_HOSTS)" "$(SIM_UI_SYNC_DURATION_MS)" "$(SIM_UI_SYNC_SAMPLE_MS)" "$$ready_file" "$(SIM_UI_SYNC_CHECKPOINT_SWEEP)"; echo "building displayed UI scenario proof test for $(APP_SCHEME) on simulator $$udid build $(APP_BUILD_NUMBER)"; if ! xcodebuild -quiet build-for-testing -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" CURRENT_PROJECT_VERSION="$(APP_BUILD_NUMBER)" > "$$build_log" 2>&1; then echo "sim UI scenario build failed; see $$build_log" >&2; tail -n 80 "$$build_log" >&2; rm -f "$$config"; exit 1; fi; ui_pid=""; cleanup() { status="$${1:-$$?}"; rm -f "$$config"; if [ -n "$${ui_pid:-}" ] && kill -0 "$$ui_pid" 2>/dev/null; then kill "$$ui_pid" 2>/dev/null || true; wait "$$ui_pid" 2>/dev/null || true; fi; exit "$$status"; }; trap cleanup INT TERM; echo "running displayed UI sampler on $(SIM) with hosts=$(SIM_UI_SYNC_HOSTS)"; xcodebuild -quiet test-without-building -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" -only-testing:CodexDockUITests/CodexDockDisplayedSyncProofTests/testSamplesRelayBackedDockDisplayOverTime > "$$test_log" 2>&1 & ui_pid="$$!"; ready_deadline=$$(( $$(date +%s) + ( $(SIM_UI_SYNC_READY_TIMEOUT_MS) / 1000 ) )); while [ ! -f "$$ready_file" ]; do if ! kill -0 "$$ui_pid" 2>/dev/null; then echo "sim UI scenario test exited before ready; see $$test_log" >&2; tail -n 120 "$$test_log" >&2; cleanup 1; fi; if [ "$$(date +%s)" -gt "$$ready_deadline" ]; then echo "sim UI scenario sampler did not become ready; see $$test_log" >&2; tail -n 120 "$$test_log" >&2; cleanup 1; fi; sleep 0.2; done; echo "running relay scenario $(SIM_UI_SYNC_SCENARIO) x$(SIM_UI_SYNC_SCENARIO_REPETITIONS) while UI sampler is active"; if ! rtk node scripts/dock-relay-sync-audit.mjs --relay-url "$(SIM_UI_SYNC_RELAY_WS)" $(SIM_UI_SYNC_CODEX_HOME_ARG) --mode scenario --scenario "$(SIM_UI_SYNC_SCENARIO)" $(SIM_UI_SYNC_SCENARIO_THREAD_ARG) --scenario-hold-ms "$(SIM_UI_SYNC_SCENARIO_HOLD_MS)" --scenario-repetitions "$(SIM_UI_SYNC_SCENARIO_REPETITIONS)" --client-path-only --detail none --dock-collection-timeout-ms 120000 --max-stream-lag-ms "$(MAX_UI_LAG_MS)" --json-out "$$relay_json" --summary-out "$$relay_md" --summary-only --fail-on-diff > "$$relay_log" 2>&1; then echo "relay scenario sync audit failed; see $$relay_log" >&2; tail -n 120 "$$relay_log" >&2; cleanup 1; fi; if ! wait "$$ui_pid"; then echo "sim UI scenario test failed; see $$test_log" >&2; tail -n 120 "$$test_log" >&2; cleanup 1; fi; ui_pid=""; rm -f "$$config"; trap - INT TERM; rtk node scripts/dock-relay-simulator-ui-sync-proof.mjs --relay-report "$$relay_json" --ui-samples "$$ui_samples" --max-ui-lag-ms "$(MAX_UI_LAG_MS)" --json-out "$$report_json" --summary-out "$$report_md" --fail-on-diff; echo "wrote $$report_json"; echo "wrote $$report_md"'

sim-ui-isolated-scenario-sync-proof:
	@rtk sh -c 'set -eu; root="$(SIM_UI_ISOLATED_ROOT)"; home="$(SIM_UI_ISOLATED_HOME)"; service_dir="$(SIM_UI_ISOLATED_SERVICE_DIR)"; metadata="$(SIM_UI_ISOLATED_METADATA)"; relay_port="$(SIM_UI_ISOLATED_RELAY_PORT)"; app_port="$(SIM_UI_ISOLATED_APP_SERVER_PORT)"; relay_ws="ws://127.0.0.1:$$relay_port"; relay_hosts="127.0.0.1:$$relay_port"; mkdir -p "$$root"; rtk node scripts/codex-dock-isolated-home.mjs create --source-home "$(SIM_UI_ISOLATED_SOURCE_HOME)" --output-home "$$home" --json-out "$$metadata" --max-rollout-bytes "$(SIM_UI_ISOLATED_MAX_ROLLOUT_BYTES)" --thread-count "$(SIM_UI_ISOLATED_THREAD_COUNT)" --force >/dev/null; thread_id="$$(rtk node -e '\''const fs = require("fs"); const data = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(data.threadID);'\'' "$$metadata")"; cleanup() { rtk make --no-print-directory host-service-stop CODEX_HOME="$$home" APP_SERVER_DIR="$$service_dir" ENV_FILE="$$service_dir/service.env" HOST_ENV_FILE="$$service_dir/host.env" APP_SERVER_PORT="$$app_port" DOCK_RELAY_PORT="$$relay_port" DOCK_RELAY_WS="$$relay_ws" CODEX_DOCK_HOSTS="$$relay_hosts" APP_SERVER_LABEL="$(SIM_UI_ISOLATED_APP_SERVER_LABEL)" DOCK_RELAY_LABEL="$(SIM_UI_ISOLATED_RELAY_LABEL)" HOST_SERVICE_NETWORK_PROFILE=simulator-local >/dev/null 2>&1 || true; }; trap cleanup EXIT INT TERM; rtk make --no-print-directory sim-ui-scenario-sync-proof SIM="$(SIM)" CODEX_HOME="$$home" APP_SERVER_DIR="$$service_dir" ENV_FILE="$$service_dir/service.env" HOST_ENV_FILE="$$service_dir/host.env" APP_SERVER_PORT="$$app_port" DOCK_RELAY_PORT="$$relay_port" DOCK_RELAY_WS="$$relay_ws" CODEX_DOCK_HOSTS="$$relay_hosts" CODEX_DOCK_UI_TEST_HOSTS="$$relay_hosts" SIM_UI_SYNC_HOSTS="$$relay_hosts" SIM_UI_SYNC_RELAY_WS="$$relay_ws" SIM_UI_SYNC_CODEX_HOME="$$home" SIM_UI_SYNC_SCENARIO_THREAD_ID="$$thread_id" SIM_UI_SYNC_DIR="$$root/sim-ui" APP_SERVER_LABEL="$(SIM_UI_ISOLATED_APP_SERVER_LABEL)" DOCK_RELAY_LABEL="$(SIM_UI_ISOLATED_RELAY_LABEL)" HOST_SERVICE_NETWORK_PROFILE=simulator-local MAX_UI_LAG_MS="$(MAX_UI_LAG_MS)" SIM_UI_SYNC_DURATION_MS="$(SIM_UI_SYNC_DURATION_MS)" SIM_UI_SYNC_SAMPLE_MS="$(SIM_UI_SYNC_SAMPLE_MS)" SIM_UI_SYNC_SCENARIO="$(SIM_UI_SYNC_SCENARIO)" SIM_UI_SYNC_SCENARIO_HOLD_MS="$(SIM_UI_SYNC_SCENARIO_HOLD_MS)" SIM_UI_SYNC_SCENARIO_REPETITIONS="$(SIM_UI_SYNC_SCENARIO_REPETITIONS)" SIM_UI_SYNC_READY_TIMEOUT_MS="$(SIM_UI_SYNC_READY_TIMEOUT_MS)" SIM_UI_SYNC_CHECKPOINT_SWEEP="$(SIM_UI_SYNC_CHECKPOINT_SWEEP)"; echo "wrote $$metadata"; echo "wrote $$root/sim-ui/simulator-ui-sync.json"; echo "wrote $$root/sim-ui/simulator-ui-sync.md"'

sim-ui-controlled-scenario-sync-proof:
	@rtk python3 scripts/sim.py boot "$(SIM)"
	@rtk xcodegen generate --spec project.yml
	@rtk sh -c 'set -eu; udid="$$(python3 scripts/sim.py resolve "$(SIM)")"; output_dir="$(SIM_UI_SYNC_DIR)"; mkdir -p "$$output_dir" "$(APP_BUILD_LOG_DIR)"; relay_json="$$output_dir/relay-client-path.json"; relay_md="$$output_dir/relay-client-path.md"; ui_samples="$$output_dir/ui-samples.jsonl"; ready_file="$$output_dir/ui-ready.json"; fixture_ready="$$output_dir/fixture-ready.json"; fixture_stop="$$output_dir/fixture-stop"; report_json="$$output_dir/simulator-ui-sync.json"; report_md="$$output_dir/simulator-ui-sync.md"; build_log="$(APP_BUILD_LOG_DIR)/sim-ui-controlled-sync-build-$(APP_BUILD_NUMBER).log"; test_log="$(APP_BUILD_LOG_DIR)/sim-ui-controlled-sync-test-$(APP_BUILD_NUMBER).log"; fixture_log="$$output_dir/controlled-fixture.out.log"; config="/tmp/codex-client/codex-dock-sim-ui-sync-config.json"; rm -f "$$ready_file" "$$fixture_ready" "$$fixture_stop"; echo "building controlled displayed UI scenario proof test for $(APP_SCHEME) on simulator $$udid build $(APP_BUILD_NUMBER)"; if ! xcodebuild -quiet build-for-testing -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" CURRENT_PROJECT_VERSION="$(APP_BUILD_NUMBER)" > "$$build_log" 2>&1; then echo "controlled sim UI build failed; see $$build_log" >&2; tail -n 80 "$$build_log" >&2; rm -f "$$config"; exit 1; fi; fixture_pid=""; cleanup() { status="$${1:-$$?}"; touch "$$fixture_stop" 2>/dev/null || true; rm -f "$$config"; if [ -n "$${fixture_pid:-}" ] && kill -0 "$$fixture_pid" 2>/dev/null; then kill "$$fixture_pid" 2>/dev/null || true; wait "$$fixture_pid" 2>/dev/null || true; fi; exit "$$status"; }; trap cleanup INT TERM; echo "starting controlled simulator fixture scenario $(SIM_UI_SYNC_SCENARIO)"; rtk node scripts/dock-relay-controlled-simulator-fixture.mjs --scenario "$(SIM_UI_SYNC_SCENARIO)" --ready-out "$$fixture_ready" --ui-ready-in "$$ready_file" --stop-in "$$fixture_stop" --json-out "$$relay_json" --summary-out "$$relay_md" --scenario-hold-ms "$(SIM_UI_SYNC_SCENARIO_HOLD_MS)" --dock-collection-timeout-ms "$(SIM_UI_SYNC_RELAY_DURATION_MS)" --max-stream-lag-ms "$(MAX_UI_LAG_MS)" --wait-timeout-ms "$(SIM_UI_SYNC_READY_TIMEOUT_MS)" > "$$fixture_log" 2>&1 & fixture_pid="$$!"; deadline=$$(( $$(date +%s) + ( $(SIM_UI_SYNC_READY_TIMEOUT_MS) / 1000 ) )); while [ ! -f "$$fixture_ready" ]; do if ! kill -0 "$$fixture_pid" 2>/dev/null; then echo "controlled simulator fixture exited before ready; see $$fixture_log" >&2; tail -n 120 "$$fixture_log" >&2; cleanup 1; fi; if [ "$$(date +%s)" -gt "$$deadline" ]; then echo "controlled simulator fixture did not become ready; see $$fixture_log" >&2; tail -n 120 "$$fixture_log" >&2; cleanup 1; fi; sleep 0.2; done; relay_hosts="$$(rtk node -e '\''const fs = require("fs"); const data = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(data.hosts);'\'' "$$fixture_ready")"; rtk node -e '\''const fs = require("fs"); const args = process.argv.slice(1); const ready = JSON.parse(fs.readFileSync(args[6], "utf8")); const iso = new Date(Date.now() + 600000).toISOString(); const expiresAt = iso.slice(0, iso.indexOf(".")) + "Z"; const config = Object.assign({ outputPath: args[1], hosts: ready.hosts, durationMS: Number(args[2]), sampleMS: Number(args[3]), readyPath: args[4], checkpointSweep: args[5] === "1", expiresAt }, ready.uiConfig || {}); fs.writeFileSync(args[0], JSON.stringify(config) + "\n");'\'' "$$config" "$$ui_samples" "$(SIM_UI_SYNC_DURATION_MS)" "$(SIM_UI_SYNC_SAMPLE_MS)" "$$ready_file" "$(SIM_UI_SYNC_CHECKPOINT_SWEEP)" "$$fixture_ready"; echo "running controlled displayed UI sampler on $(SIM) with hosts=$$relay_hosts"; if ! xcodebuild -quiet test-without-building -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" -only-testing:CodexDockUITests/CodexDockDisplayedSyncProofTests/testSamplesRelayBackedDockDisplayOverTime > "$$test_log" 2>&1; then echo "controlled sim UI test failed; see $$test_log" >&2; tail -n 120 "$$test_log" >&2; cleanup 1; fi; touch "$$fixture_stop"; if ! wait "$$fixture_pid"; then echo "controlled simulator fixture failed; see $$fixture_log" >&2; tail -n 120 "$$fixture_log" >&2; rm -f "$$config"; exit 1; fi; fixture_pid=""; rm -f "$$config"; trap - INT TERM; rtk node scripts/dock-relay-simulator-ui-sync-proof.mjs --relay-report "$$relay_json" --ui-samples "$$ui_samples" --max-ui-lag-ms "$(MAX_UI_LAG_MS)" --json-out "$$report_json" --summary-out "$$report_md" --fail-on-diff; echo "wrote $$relay_json"; echo "wrote $$relay_md"; echo "wrote $$report_json"; echo "wrote $$report_md"'
...
sim-ui-controlled-matrix-proof:
	@rtk sh -c 'set -eu; root="$(SIM_UI_CONTROLLED_MATRIX_ROOT)"; scenarios="$(SIM_UI_CONTROLLED_MATRIX_SCENARIOS)"; passes="$(SIM_UI_CONTROLLED_MATRIX_PASSES)"; if [ "$$passes" -lt 1 ]; then echo "SIM_UI_CONTROLLED_MATRIX_PASSES must be at least 1" >&2; exit 2; fi; mkdir -p "$$root"; report_dirs=""; pass=1; while [ "$$pass" -le "$$passes" ]; do for scenario in $$scenarios; do duration="$(SIM_UI_SYNC_DURATION_MS)"; hold="$(SIM_UI_SYNC_SCENARIO_HOLD_MS)"; case "$$scenario" in detail-history-request) duration="20000";; large-list-checkpoint) duration="14000";; thread-activity) duration="18000";; server-request) duration="12000";; rapid-mutations) duration="28000"; hold="2500";; multi-host-isolation) duration="16000";; source-refresh|live-lease-expiry|spawn-edge|resync-gap) duration="14000";; *) echo "unknown controlled simulator matrix scenario $$scenario" >&2; exit 2;; esac; scenario_dir="$$root/pass-$$pass/$$scenario"; echo "running controlled simulator matrix pass $$pass scenario $$scenario -> $$scenario_dir"; rtk make --no-print-directory sim-ui-controlled-scenario-sync-proof SIM="$(SIM)" SIM_UI_SYNC_SCENARIO="$$scenario" SIM_UI_SYNC_DIR="$$scenario_dir" SIM_UI_SYNC_DURATION_MS="$$duration" SIM_UI_SYNC_SAMPLE_MS="$(SIM_UI_CONTROLLED_MATRIX_SAMPLE_MS)" SIM_UI_SYNC_SCENARIO_HOLD_MS="$$hold" MAX_UI_LAG_MS="$(MAX_UI_LAG_MS)" SIM_UI_SYNC_CHECKPOINT_SWEEP="$(SIM_UI_SYNC_CHECKPOINT_SWEEP)" SIM_UI_SYNC_READY_TIMEOUT_MS="$(SIM_UI_SYNC_READY_TIMEOUT_MS)" SIM_UI_SYNC_RELAY_DURATION_MS="$(SIM_UI_SYNC_RELAY_DURATION_MS)"; report_dirs="$$report_dirs $$scenario_dir"; done; pass=$$((pass + 1)); done; matrix_json="$$root/controlled-simulator-matrix.json"; matrix_md="$$root/controlled-simulator-matrix.md"; rtk make --no-print-directory sim-ui-controlled-matrix-verify SIM_UI_MATRIX_REPORT_DIRS="$$report_dirs" SIM_UI_MATRIX_JSON="$$matrix_json" SIM_UI_MATRIX_MD="$$matrix_md" SIM_UI_MATRIX_MIN_PASSES="$$passes" MAX_UI_LAG_MS="$(MAX_UI_LAG_MS)"; echo "wrote $$matrix_json"; echo "wrote $$matrix_md"'
