...
env-file:
	@rtk sh -c 'set -eu; { echo "CODEX_DOCK_PHONE_REACHABLE_APP_SERVER_WS=$(DOCK_RELAY_WS)"; echo "CODEX_DOCK_APP_SERVER_BEARER_TOKEN_FILE=$(APP_SERVER_TOKEN)"; echo "CODEX_DOCK_REAL_HOST_ID=Amir-M5"; echo "CODEX_DOCK_REAL_HOST_NAME=Amir-M5"; } > "$(ENV_FILE)"; echo "wrote $(ENV_FILE)"'
...
app: services
	...
	@rtk sh -c 'set -eu; udid="$$(python3 scripts/sim.py resolve "$(SIM)")"; echo "building $(APP_SCHEME) for $$udid"; xcodebuild -quiet -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" build; echo "installing $(APP_PATH)"; xcrun simctl install "$$udid" "$(APP_PATH)"; token="$$(cat "$(APP_SERVER_TOKEN)")"; SIMCTL_CHILD_CODEX_DOCK_PHONE_REACHABLE_APP_SERVER_WS="$(DOCK_RELAY_WS)" SIMCTL_CHILD_CODEX_DOCK_APP_SERVER_BEARER_TOKEN="$$token" SIMCTL_CHILD_CODEX_DOCK_REAL_HOST_ID=Amir-M5 SIMCTL_CHILD_CODEX_DOCK_REAL_HOST_NAME=Amir-M5 xcrun simctl launch --terminate-running-process "$$udid" "$(APP_BUNDLE_ID)"; echo "launched $(APP_BUNDLE_ID) on $$udid"'
