...
CODEX_DOCK_HOST_HOME_BEARER_TOKEN ?=
CODEX_DOCK_OPENAI_TRANSCRIPTION_MODEL ?= gpt-4o-transcribe

.DEFAULT_GOAL := help

.PHONY: help app services env-file node-deps app-server app-server-status app-server-env app-server-stop app-server-restart dock-relay dock-relay-status dock-relay-stop dock-relay-restart sims sim sim-list sim-boot run
...
services: app-server dock-relay env-file

env-file:
	@rtk sh -c 'set -eu; openai_key="$${OPENAI_API_KEY:-}"; if [ -z "$$openai_key" ] && [ -f "$(ENV_FILE)" ]; then openai_key="$$(awk -F= '\''$$1=="OPENAI_API_KEY"{sub(/^[^=]*=/,""); print; exit}'\'' "$(ENV_FILE)")"; fi; { echo "CODEX_DOCK_HOSTS=$(CODEX_DOCK_HOSTS)"; echo "CODEX_DOCK_HOST_AMIR_M5_WS=$(CODEX_DOCK_HOST_AMIR_M5_WS)"; echo "CODEX_DOCK_HOST_AMIR_M5_BEARER_TOKEN_FILE=$(APP_SERVER_TOKEN)"; echo "CODEX_DOCK_HOST_AMIR_M5_NAME=$(CODEX_DOCK_HOST_AMIR_M5_NAME)"; echo "CODEX_DOCK_HOST_HOME_WS=$(CODEX_DOCK_HOST_HOME_WS)"; echo "CODEX_DOCK_HOST_HOME_BEARER_TOKEN_FILE=$(APP_SERVER_TOKEN)"; echo "CODEX_DOCK_HOST_HOME_NAME=$(CODEX_DOCK_HOST_HOME_NAME)"; echo "CODEX_DOCK_PHONE_REACHABLE_APP_SERVER_WS=$(DOCK_RELAY_WS)"; echo "CODEX_DOCK_APP_SERVER_BEARER_TOKEN_FILE=$(APP_SERVER_TOKEN)"; echo "CODEX_DOCK_REAL_HOST_ID=Amir-M5"; echo "CODEX_DOCK_REAL_HOST_NAME=Amir-M5"; echo "CODEX_DOCK_OPENAI_TRANSCRIPTION_MODEL=$(CODEX_DOCK_OPENAI_TRANSCRIPTION_MODEL)"; if [ -n "$$openai_key" ]; then echo "OPENAI_API_KEY=$$openai_key"; fi; } > "$(ENV_FILE)"; echo "wrote $(ENV_FILE)"'
...
app-server-env:
	@rtk sh -c 'set -eu; echo "export CODEX_DOCK_PHONE_REACHABLE_APP_SERVER_WS=$(DOCK_RELAY_WS)"; echo "export CODEX_DOCK_APP_SERVER_BEARER_TOKEN_FILE=$(APP_SERVER_TOKEN)"; echo "export CODEX_DOCK_REAL_HOST_ID=Amir-M5"; echo "export CODEX_DOCK_REAL_HOST_NAME=Amir-M5"'
...
app: services
	...
	@rtk sh -c 'set -eu; udid="$$(python3 scripts/sim.py resolve "$(SIM)")"; echo "building $(APP_SCHEME) for $$udid"; xcodebuild -quiet -project CodexDock.xcodeproj -scheme "$(APP_SCHEME)" -destination "id=$$udid" -derivedDataPath "$(APP_DERIVED_DATA)" build; echo "installing $(APP_PATH)"; xcrun simctl install "$$udid" "$(APP_PATH)"; token="$$(cat "$(APP_SERVER_TOKEN)")"; home_token="$(CODEX_DOCK_HOST_HOME_BEARER_TOKEN)"; if [ -z "$$home_token" ]; then home_token="$$token"; fi; openai_key="$${OPENAI_API_KEY:-}"; if [ -z "$$openai_key" ] && [ -f "$(ENV_FILE)" ]; then openai_key="$$(awk -F= '\''$$1=="OPENAI_API_KEY"{sub(/^[^=]*=/,""); print; exit}'\'' "$(ENV_FILE)")"; fi; echo "launch endpoint: $(DOCK_RELAY_WS)"; echo "launch hosts: $(CODEX_DOCK_HOSTS)"; SIMCTL_CHILD_CODEX_DOCK_HOSTS="$(CODEX_DOCK_HOSTS)" SIMCTL_CHILD_CODEX_DOCK_HOST_AMIR_M5_WS="$(CODEX_DOCK_HOST_AMIR_M5_WS)" SIMCTL_CHILD_CODEX_DOCK_HOST_AMIR_M5_BEARER_TOKEN="$$token" SIMCTL_CHILD_CODEX_DOCK_HOST_AMIR_M5_NAME="$(CODEX_DOCK_HOST_AMIR_M5_NAME)" SIMCTL_CHILD_CODEX_DOCK_HOST_HOME_WS="$(CODEX_DOCK_HOST_HOME_WS)" SIMCTL_CHILD_CODEX_DOCK_HOST_HOME_BEARER_TOKEN="$$home_token" SIMCTL_CHILD_CODEX_DOCK_HOST_HOME_NAME="$(CODEX_DOCK_HOST_HOME_NAME)" SIMCTL_CHILD_CODEX_DOCK_PHONE_REACHABLE_APP_SERVER_WS="$(DOCK_RELAY_WS)" SIMCTL_CHILD_CODEX_DOCK_APP_SERVER_BEARER_TOKEN="$$token" SIMCTL_CHILD_CODEX_DOCK_REAL_HOST_ID=Amir-M5 SIMCTL_CHILD_CODEX_DOCK_REAL_HOST_NAME=Amir-M5 SIMCTL_CHILD_CODEX_DOCK_OPENAI_TRANSCRIPTION_MODEL="$(CODEX_DOCK_OPENAI_TRANSCRIPTION_MODEL)" SIMCTL_CHILD_OPENAI_API_KEY="$$openai_key" xcrun simctl launch --terminate-running-process "$$udid" "$(APP_BUNDLE_ID)"; echo "launched $(APP_BUNDLE_ID) on $$udid"'
